...
fftSize = 512; % FFT LENGTH
factorX = 2; 
BIT_LENGTH = 24; %I2S WORD BIT LENGTH
FULL_SCALE_DBSPL = 120; %% FULL SCALE in dBSPL (AOP = 116dB SPL)
FULL_SCALE_DBFS = 20*log10((2^(BIT_LENGTH))); %% FULL SCALE in dBFS
RMS_HANN = 0.61177; %% RMS FOR HANN WINDOW
CONST_FACTOR = 64; %% DOWNSCALING FACTOR TO PREVENT OVERFLOW
FIR_COEFS_LENGTH = 200;
...
[freq_ampl_wtable,ampl_wtable]=aweightingfn(fs,fftSize-1);
%[freq_ampl_wtable,ampl_wtable]=cweightingfn(fs,fftSize-1);
ampl_wtable_24bit = 10.^(ampl_wtable/20);

%RETRIEVE DATA FROM WHITE NOISE EQUALISATION
Equalisation = csvread("../Filter_Equalization.csv");
freq_equalization_filter_aux = real(Equalisation)'; %FREQUENCY COLUMN
freq_equalization_filter = freq_equalization_filter_aux(2:end); %IGNORE THE FIRST LINE
ampl_equalization_filter_aux = imag(Equalisation)'; %AMPLITUDE COLUMN
ampl_equalization_filter = ampl_equalization_filter_aux(2:end); %IGNORE THE FIRST LINE
%X-AXIS
vector_fir = [0:1:length(ampl_wtable_24bit)-1]./(length(ampl_wtable_24bit)-1);
...
filter_desired = ampl_wtable_24bit./ampl_equalization_filter_resampled;
%GENERATE FIR FILTER COEFFICIENTS WITH FIR2 FUNCTION
FIR_COEFS = fir2(FIR_COEFS_LENGTH,vector_fir,filter_desired);
[h, w] = freqz (FIR_COEFS);
freq_FIR_COEF = w/pi;
ampl_FIR_COEF = abs (h);
%FLIP THEM (NEVERMIND, THEY ARE SYMMETRIC)
FIR_COEFS_FLIPPED = fliplr(FIR_COEFS);

%% AUTO-GENERATED H FILES
%H
filename = "../../lib/AudioAnalysis/FilterCoefficients/fir_coeffs.h";
...
fputs (fid, disp(""));
fputs (fid, disp("const uint8_t FILTERSIZE = FIR_COEFFS_LEN;"));
fputs (fid, disp("const uint8_t FILTERBLOCKSIZE = 15;"));
...
%% FILTER (time domain) - JUST FOR THE FUN OF CHECKING 
...
%% COMPARISON
%APPLY FILTER ON FREQUENCY DOMAIN
p2_filtered = filter_desired.* p2;
%APPLY FILTER ON TIME DOMAIN
...
%APPLY FILTER ON TIME DOMAIN (SEPARATE REAL AND IMAG)
...
%APPLY THE FILTER ON TIME DOMAIN BY CONVOLUTION
...
semilogx(axisFIR_COEFS_fs,abs(h),'b',freq_ampl_wtable,filter_desired,'r',freq_ampl_wtable,ampl_wtable_24bit,'--k',freq_ampl_wtable,ampl_equalization_filter_resampled,'g');
xlabel('Frequency (-)')
ylabel('Filter (-)')
title('Frequency domain filter')
...
figure
axisFIR_COEFS_fs = w/pi * fs/2;
semilogx(axisFIR_COEFS_fs,abs(h),'b',freq_ampl_wtable,filter_desired,'r');
xlabel('Frequency (-)')
ylabel('Filter (-)')
title('Frequency domain filter')
...
figure
subplot(2,1,1)
plot(freq_ampl_wtable,filter_desired)
xlabel('Frequency (-)')
ylabel('Filter (-)')
title('Frequency domain filter')
grid
...
subplot(2,1,2)
plot(freq_ampl_wtable,p2,'b',freq_ampl_wtable,p2_filtered,'r')
xlabel('Frequency (-)')
...
figure
plot(freq_ampl_wtable,p2,'b',freq_ampl_wtable,p2_filtered,'r',freq_ampl_wtable,p2yfilter_cor,'k',freq_ampl_wtable,p2yfilter_cor_RI,'--g',freq_ampl_wtable,p2yfilter_cor_FIR,'--m')
xlabel('Frequency (-)')
ylabel('Signal frequency response (-)')
